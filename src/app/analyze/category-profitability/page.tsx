...
export default function CategoryProfitabilityPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/data/profitability?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}${marketplaceParam}&groupBy=category`);
  ...
  const columns = useMemo<ColumnDef<ProfitRow, any>[]>(() => [
    {
      ...
      cell: ({ getValue }) => <span className="text-sm text-text-primary font-medium">{getValue() as string || 'Uncategorized'}</span>,
      size: 200,
    },
    {
      id: 'orders',
  ...
  function handleExport() {
    const headers = ['Category', 'Orders', 'Units Sold', 'Refunds', 'Revenue', 'Cost/Unit', 'Fees', 'On Hand', 'Profit', 'ROI %', 'Margin %'];
    ...
    a.href = url;
    a.download = 'category-profitability.csv';
    a.click();
  ...
  const footerRow = totals ? {
    category: <span className="text-text-primary font-semibold">Totals</span>,
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Analyze > Category Profitability"
        dateRange={dateRange}
